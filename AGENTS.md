...
- **先写测试，再写代码** — 核心逻辑在交付前必须有测试覆盖
- **本地可跑** — `bash tests/test.sh` 能在任何时候验证代码健康
- **测试覆盖率目标** — 核心业务逻辑 100%，边界情况全覆盖
- **测试分层**：
  - 单元测试：纯函数逻辑（test_score.sh）
  - 数据结构测试：JSON 输出格式（test_collector.sh）
  - 集成测试：真实命令调用（test_git_status.sh）
- **测试失败 = 代码未完成** — 不存在"测试挂了但功能正常"这种说法
- **每次 commit 前跑测试** — 确保没有引入新的问题
